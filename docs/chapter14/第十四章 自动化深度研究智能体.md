...
其中 `keenable` 后端调用 [Keenable](https://keenable.ai) 的公开搜索接口，不需要申请 API 密钥，克隆项目后把 `SEARCH_API` 设为 `keenable` 即可使用。它是这里唯一一个无需密钥就能工作的搜索 API 选项（DuckDuckGo 后端同样不需要密钥，但它依赖抓取搜索结果页，而不是调用正式接口）。可选的 `KEENABLE_API_KEY` 只用于提高按 IP 计算的请求频率上限。由于 `hello-agents` 0.2.9 的 `SearchTool` 只支持固定的几个后端，这个后端实现在 `services/keenable_search.py` 中，由 `dispatch_search` 负责分发；它返回的字典结构与 `SearchTool` 完全一致，因此下面的去重和格式化逻辑无需改动。
