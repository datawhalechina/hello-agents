...
│   │   ├── client.py                   # MCP客户端（stdio / Streamable HTTP）
...
<strong>（3）MCP 的机制：能力协商，而非“把工具自动交给模型”</strong>

MCP 在消息层使用 UTF-8 编码的 JSON-RPC 2.0。连接建立后，Client 必须先发送`initialize`，双方在响应中协商协议版本和能力；Client 再发送`initialized`通知，才进入正常运行阶段。断开连接或关闭底层传输即结束会话。

MCP 把能力按方向分开声明，未声明的能力不能假定可用：

| 提供方 | 机制 | 含义 |
| --- | --- | --- |
| Server → Client | <strong>Tools</strong> | 以名称、说明和 JSON Schema 暴露可执行操作；通过`tools/list`发现、`tools/call`调用。工具清单可变更并通知 Client。 |
| Server → Client | <strong>Resources</strong> | 以 URI 暴露可读取的上下文数据，也可提供资源模板与订阅更新；它们不会自动塞入模型上下文。 |
| Server → Client | <strong>Prompts</strong> | 可发现、可填参的提示模板；是否采用以及如何展示由 Host 决定。 |
| Client → Server | <strong>Roots</strong> | Client 告知 Server 可操作的本地文件边界；Server 必须尊重边界，不能把它当作任意文件访问授权。 |
| Client → Server | <strong>Sampling / Elicitation</strong> | Server 可请求 Host 代为调用模型（Sampling），或向用户收集信息（Elicitation）；模型、权限和交互界面始终由 Client/Host 控制。 |

此外，协议还定义了`ping`、进度通知和取消等通用机制。它规定消息、能力和边界，<strong>不规定</strong>模型必须调用哪个工具、Host 如何将工具映射到某家模型的 function calling，或是否跳过用户确认。

<strong>（4）一次工具调用的实际流程</strong>

1. Client 与 Server 完成`initialize` / `initialized`，并记录双方声明的能力。
2. Client 在支持工具能力时请求`tools/list`；Host 可将工具 schema 映射为模型提供商的工具格式，也可以只显示给用户。
3. 在 Host 的策略允许、模型（或用户）选择某工具后，Client 发送`tools/call`。敏感操作应在这里展示参数并征得用户确认。
4. Server 校验输入和权限后执行，返回内容或结构化结果；业务执行失败应作为工具结果（`isError: true`）返回，协议层错误则使用 JSON-RPC error。
5. Host 决定是否把结果交回模型并继续下一轮。因而“发现工具 → 模型调用 → 结果回填”是常见的 Host 实现模式，不是 MCP 强制的自动化行为。

<strong>安全边界：</strong>远程 Server 的访问控制属于传输层；HTTP 场景可采用 MCP 的 OAuth 2.1 授权规范，stdio 通常从受控环境读取凭据。无论哪种方式，都应最小权限、校验输入/输出、限制可访问的 roots，并对写入、删除、发送等高风险工具保留用户审批和审计记录。
...
### 10.2.3 当前 MCP 传输方式：两种标准，其他是实现细节

MCP 的消息格式与传输解耦，但<strong>当前规范只定义两种标准 Client–Server 传输</strong>：`stdio` 和 `Streamable HTTP`。`Memory transport`只是测试替身；“带参数的 stdio”仍是 stdio；SSE 是 Streamable HTTP 可选使用的事件流，并不是第三种传输。

| 标准传输 | 适合场景 | 传输语义与注意事项 |
| --- | --- | --- |
| <strong>stdio</strong> | 本机集成、IDE/桌面应用启动的插件、开发调试 | Client 启动 Server 子进程；请求、通知和响应以换行分隔的 JSON-RPC 在 stdin/stdout 上传递。stdout 只能写协议消息，日志写 stderr。进程生命周期就是连接生命周期。 |
| <strong>Streamable HTTP</strong> | 远程服务、多客户端、生产部署 | Server 提供一个同时支持 POST/GET 的 MCP endpoint（如`https://api.example.com/mcp`）。POST 承载 Client 消息；GET 或 POST 响应可选用 SSE 流式返回多条 Server 消息，因此能支持通知、Server 发起的请求与断线恢复。可用`MCP-Session-Id`维护有状态会话。 |

<strong>历史兼容性：</strong>2024-11-05 规范中的 <code>HTTP + SSE</code>（独立 SSE endpoint 加 POST endpoint）已废弃。新 Server 应实现 Streamable HTTP；为了兼容旧 Client 可以额外保留旧 endpoint。不要把普通 REST HTTP、WebSocket 或某个 SDK 的“HTTP transport”名称直接当作 MCP 标准传输——它们要么是在实现 Streamable HTTP，要么是自定义传输，互操作性取决于双方约定。

<strong>（1）stdio：本地子进程</strong>

```python
from hello_agents.tools import MCPTool

# Client 启动本地 Server；命令参数不是另一种传输方式
mcp_tool = MCPTool(
...
stdio 默认没有网络认证流程：Server 应从受控的进程环境取得必要凭据，且绝不能把调试日志写入 stdout 破坏消息流。

<strong>（2）Streamable HTTP：远程 MCP endpoint</strong>
...
async def use_remote_server():
    # 具体参数名以所用 SDK 版本为准；目标是 Streamable HTTP endpoint
    client = MCPClient(
...
远程部署必须使用 HTTPS，并验证每个请求的 <code>Origin</code> 以防 DNS rebinding；本地 HTTP Server 应只绑定 loopback 地址。若 Server 受保护，应按 MCP 的 OAuth 2.1 授权流程发现授权服务器并在每次 HTTP 请求携带访问令牌；令牌不能放进 URL，也不能透传给下游服务。会话 ID 不是认证凭据，仍须安全保存和校验。

<strong>选择建议：</strong>能由 Host 启动、只服务当前用户时优先 stdio；需要独立部署或让多个 Client 访问时使用 Streamable HTTP。只有在兼容旧服务时才实现旧 HTTP+SSE；自定义传输必须自行定义认证、消息边界和重连语义。
...
2. MCP（Model Context Protocol）是智能体与工具通信的标准协议。基于 10.2 节的内容，请深入思考：
   ...
   - MCP 使用 JSON-RPC 2.0 消息，并标准化了 stdio 与 Streamable HTTP 两种传输。请比较它们的优势、局限性与安全边界；再说明旧 HTTP+SSE 的兼容策略，以及若采用 WebSocket 等自定义传输，需要自行补齐哪些认证、消息边界与重连语义。
