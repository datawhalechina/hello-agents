...
我们创建了一个真实的天气查询 MCP 服务器。现在，让我们先准备标准的发布目录，再分别了解如何将它部署到托管平台 Render，以及如何提交到 Smithery 平台，让更多开发者能够使用我们的服务。
...
Smithery 更像 MCP 服务器的目录和分发平台，而 Render 则是一个通用的云端托管平台。将 MCP 服务器部署到 Render 后，我们会得到一个稳定的 HTTPS 地址，例如：

```text
https://weather-mc-server-demo-77tg.onrender.com
```

这个地址可以直接作为远程 MCP Endpoint 使用，适合团队内部测试、私有工具服务、演示项目，或者后续再提交到 Smithery 时作为可访问的线上服务。

<strong>第一步：在 Render 创建 Web Service</strong>

打开 [https://render.com/](https://render.com/)，使用 GitHub 账号登录后，按照下面的步骤创建服务：

1. 点击 Dashboard 右上角的 <strong>New</strong>，选择 <strong>Web Service</strong>。

<div align="center">
  <img src="https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/10-figures/10-10.png" alt="" width="85%"/>
  <p>图 10.10 Render 创建 Web Service </p>
</div>

2. 选择 <strong>Git Provider</strong>，连接 GitHub，并选择刚刚创建的 `weather-mcp-server` 仓库。

<div align="center">
  <img src="https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/10-figures/10-11.png" alt="" width="85%"/>
  <p>图 10.11 Render 连接Github 仓库 </p>
</div>

3. 在服务配置页面填写：

- <strong>Name</strong>：`weather-mcp-server`
- <strong>Region</strong>：选择离主要用户更近的区域，例如 Singapore、Oregon 等
- <strong>Branch</strong>：`main`
- <strong>Language</strong>：选择 `Docker`
- <strong>Dockerfile Path</strong>：如果 Dockerfile 位于仓库根目录，保持默认 `./Dockerfile`
- <strong>Docker Context</strong>：保持默认仓库根目录

<div align="center">
  <img src="https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/10-figures/10-11.png" alt="" width="85%"/>
  <p>图 10.12 Render 配置界面 </p>
</div>

4. 在 <strong>Environment Variables</strong> 中添加：

- `HOST=0.0.0.0`
- `PORT=10000`
- 如果你的 MCP 服务器调用外部 API，再添加对应的密钥，例如 `WEATHER_API_KEY`

5. 在 <strong>Health Check Path</strong> 中可以先留空，让 Render 使用默认 TCP 健康检查。若后续为服务增加 `/health` 路由，则可以填写 `/health`，让 Render 通过 HTTP 状态码判断服务是否健康。
6. 选择实例规格。教学或演示可以先使用免费，生产环境建议选择付费实例，避免冷启动和资源限制影响 MCP 调用体验。
7. 点击 <strong>Deploy Web Service</strong>，Render 会自动拉取代码、构建 Docker 镜像并启动服务。

部署过程中可以打开 Render 服务页面的 <strong>Logs</strong> 查看构建与运行日志。正常情况下，你会看到依赖安装、镜像构建、容器启动，以及 `Starting Weather MCP Server...` 之类的输出。如果部署失败，日志通常会直接说明原因，例如依赖安装失败、端口未监听、进程启动后立即退出等。部署成功后，Render 会为服务分配一个 `onrender.com` 域名。假设服务名为 `weather-mcp-server`，线上 MCP Endpoint 通常是：
```text
https://weather-mc-server-demo-77tg.onrender.com
```
完成这一步后，你就拥有了一个云端可访问的 MCP 服务器。接下来既可以把这个地址提供给内部客户端使用，也可以继续提交到 Smithery，让社区用户通过目录发现并安装。
<div align="center">
  <img src="https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/10-figures/10-13.png" alt="" width="85%"/>
  <p>图 10.13 Render 部署成功界面 </p>
</div>

（4）提交到 Smithery

打开浏览器，访问 [https://smithery.ai/](https://smithery.ai/)。使用 GitHub 账号登录 Smithery。点击页面上的 "Publish Server" 按钮，输入你的 Render 地址 URL：`https://weather-mc-server-demo-77tg.onrender.com/mcp`，即可等待发布。

一旦发布完成，可以看到类似这样的页面，如图 10.14 所示：

<div align="center">
  <img src="https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/10-figures/10-14.png" alt="" width="85%"/>
  <p>图 10.14 Smithery 发布成功页面 </p>
</div>
...
当然，这里只是举例，还有更多的用法可以自行探索，下图 10.15 展示了当 MCP 工具发布成功会包含的信息，显示服务的名称“天气”，其唯一标识符 `@jjyaoao/weather-mcp-server`，以及状态信息。Tools 区域就是我们刚刚实现的方法，Connect 区则提供了连接和使用此服务所需的技术信息，包括服务的<strong>接入 URL 地址</strong>和多种语言/环境下的<strong>配置代码片段</strong>。如果想要更加深入了解可以点击这个[链接](https://smithery.ai/server/@jjyaoao/weather-mcp-server)。

<div align="center">
  <img src="https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/10-figures/10-15.png" alt="" width="85%"/>
  <p>图 10.15 Smithery 发布成功的 MCP 工具 </p>
</div>
