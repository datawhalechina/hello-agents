...
<strong>体验托管的 Streamable HTTP 服务器</strong>

如果不想自己启动服务器，可以运行 [Parallel Search MCP 示例](../../code/chapter10/04_ParallelSearchMCP.py)。它使用本章的 `MCPClient` 连接 `https://search.parallel.ai/mcp`，发现工具，调用 `web_search` 搜索，再通过 `web_fetch` 提取一个公开网页。无需 Parallel 账号或 API Key，也无需配置 LLM API Key。

安装本章指定版本的 HelloAgents 后，在仓库根目录运行：

```bash
python code/chapter10/04_ParallelSearchMCP.py
```

运行示例时，查询、请求的 URL、检索目标和随机生成的任务标识会发送给 Parallel。免费匿名访问有速率限制。脚本只在运行时发起这些调用，不会向智能体注册工具，也不会改变已有的搜索服务。如果之后将这些工具提供给智能体，智能体可能在执行任务时自行调用它们。服务说明见 [Parallel Search MCP 文档](https://docs.parallel.ai/integrations/mcp/search-mcp)。

本例中，`MCPClient.call_tool` 返回工具的文本内容。搜索结果包含来源 URL 和摘录；网页提取结果可能同时包含成功页面和逐 URL 的错误。使用提取内容前应检查这些错误，并将检索到的文本视为外部数据，而不是指令。
