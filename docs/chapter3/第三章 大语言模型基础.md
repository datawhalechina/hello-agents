...
把三个门放到同一个时间步中看，LSTM 的信息更新可以概括为：先由遗忘门从旧记忆 $C_{t-1}$ 中筛掉不再需要的信息，再由输入门写入当前候选信息，得到新的细胞状态 $C_t$，最后由输出门决定 $C_t$ 的哪些部分形成隐藏状态 $h_t$。其中最关键的一步是：

$$C_t=f_t\odot C_{t-1}+i_t\odot \widetilde{C}_t$$

这里的 $f_t$、$i_t$ 分别是遗忘门和输入门的输出，$\odot$ 表示逐元素相乘。与标准 RNN 需要让信息反复穿过非线性变换不同，细胞状态包含一条以“加法”为主的信息通路，使梯度更容易沿时间传播，因此能缓解梯度消失并保留更久以前的信息。需要注意，LSTM 是<strong>缓解</strong>而不是彻底消除长期依赖问题；梯度裁剪可以控制梯度爆炸，但也不能解决梯度消失和串行计算的瓶颈。
...
<strong>一个可以手算的注意力矩阵例子</strong>

为了看清公式中的“相加”发生在哪里，假设序列只有两个词元，且：

$$Q=K=\begin{bmatrix}1&0\\0&1\end{bmatrix},\qquad
V=\begin{bmatrix}1&0\\0&2\end{bmatrix},\qquad d_k=2$$

第一步，$QK^T/\sqrt{d_k}$ 得到一个 $2\times2$ 的分数矩阵。<strong>每一行对应一个 Query 对所有 Key 的分数</strong>，而不是“每一行只有一个分数”：

$$S=\frac{QK^T}{\sqrt{2}}=
\begin{bmatrix}0.707&0\\0&0.707\end{bmatrix}$$

第二步，对每一行分别做 Softmax，得到注意力权重：

$$A=\text{softmax}(S)\approx
\begin{bmatrix}0.670&0.330\\0.330&0.670\end{bmatrix}$$

例如第一行 $[0.670,0.330]$ 表示：第一个 Query 生成新表示时，分别使用 67.0% 和 33.0% 的权重读取两个 Value。第三步是普通的矩阵乘法 $O=AV$：

$$O\approx
\begin{bmatrix}0.670&0.330\\0.330&0.670\end{bmatrix}
\begin{bmatrix}1&0\\0&2\end{bmatrix}
=\begin{bmatrix}0.670&0.660\\0.330&1.340\end{bmatrix}$$

以第一行输出为例，它等于 $0.670\times[1,0]+0.330\times[0,2]=[0.670,0.660]$。因此，所谓“加权求和”不是把权重矩阵中的数字彼此相加，而是<strong>用一整行权重逐个缩放对应的 Value 向量，再把这些向量相加</strong>。输出仍有两行，因为两个 Query 都各自得到一个融合上下文后的新表示。
...
<strong>把多头注意力的维度变化串起来</strong>

设批大小为 $B$，Query 序列长度为 $L_q$，Key/Value 序列长度为 $L_k$，模型维度为 $d_{model}$，头数为 $h$，则每个头的维度 $d_k=d_{model}/h$。以上代码中的形状依次为：

| 阶段 | 张量形状 | 含义 |
| --- | --- | --- |
| 输入与线性投影后的 $Q$ | $(B,L_q,d_{model})$ | 每个位置仍由一个完整向量表示 |
| 拆头并转置后的 $Q$ | $(B,h,L_q,d_k)$ | 把表示维度分给 $h$ 个头 |
| 拆头并转置后的 $K,V$ | $(B,h,L_k,d_k)$ | 每个头独立读取 Key 和 Value |
| 分数矩阵 $QK^T$ | $(B,h,L_q,L_k)$ | 每个头中，每个 Query 对每个 Key 都有一个分数 |
| Softmax 后的权重 | $(B,h,L_q,L_k)$ | 对最后一维 $L_k$ 归一化，每一行权重之和为 1 |
| 单头加权结果 | $(B,h,L_q,d_k)$ | 每个 Query 在每个头中得到一个输出向量 |
| 转置并拼接 | $(B,L_q,h\cdot d_k)$ | 把各头结果沿特征维拼回去 |
| 输出投影 $W^O$ | $(B,L_q,d_{model})$ | 融合各头信息，恢复模型维度 |

“拆头”不是把词元分成几组，也不是把序列长度切短，而是把<strong>每个词元的特征维度</strong>分成 $h$ 份。各头独立计算后沿特征维拼接，所以 `concat` 后又回到 $d_{model}$。

<strong>Mask 在分数矩阵的什么位置生效</strong>

Mask 会在 Softmax <strong>之前</strong>作用于 $QK^T/\sqrt{d_k}$。被遮挡位置会被替换为 $-\infty$（实现中常用 `-1e9`），这样经过 Softmax 后权重就变为 0。Transformer 中常见的两类 Mask 解决的是不同问题：

- <strong>Padding Mask</strong>：同一批次中的句子长度不同，补齐用的 `<PAD>` 不含语义。它屏蔽所有 Query 对这些 Key 位置的关注，常见的可广播形状是 $(B,1,1,L_k)$。
- <strong>Causal Mask（因果掩码）</strong>：生成第 $t$ 个位置时不能看到 $t$ 之后的答案。它是一个上三角遮挡矩阵，常见形状是 $(1,1,L_q,L_k)$。

训练 Decoder 时两种 Mask 往往会同时存在：Padding Mask 处理“补齐位置”，Causal Mask 处理“未来位置”，合并后只要任一条件要求遮挡，该位置的注意力权重就应为 0。

<strong>Self-Attention 与 Cross-Attention 的区别</strong>

二者使用完全相同的注意力公式，区别在于 $Q,K,V$ 来自哪里：

| 类型 | Query 来源 | Key / Value 来源 | 分数矩阵形状 |
| --- | --- | --- | --- |
| Encoder Self-Attention | 编码器当前序列 | 同一个编码器序列 | $(L_{src},L_{src})$ |
| Decoder Masked Self-Attention | 解码器已生成序列 | 同一个解码器序列 | $(L_{tgt},L_{tgt})$ |
| Decoder Cross-Attention | 解码器当前状态 | 编码器输出 | $(L_{tgt},L_{src})$ |

在机器翻译中，Decoder 生成目标词时，用自己的当前状态构造 Query，再到 Encoder 对源句子的表示中查找相关信息。例如生成中文“智能体”时，Cross-Attention 可以让该位置重点读取英文源句中 “agent” 的 Value。因而，Self-Attention 负责建模“本序列内部的关系”，Cross-Attention 负责连接“正在生成的目标序列”和“已经编码好的输入序列”。
...
<strong>（6）Transformer 如何用于图像：ViT</strong>

Transformer 最初处理的是词元序列，但它的注意力机制并不要求输入必须是文字。<strong>Vision Transformer (ViT)</strong> 的核心做法是把一张图片转换成“图像块序列”<sup>[15]</sup>：先把大小为 $H\times W\times C$ 的图片切成 $P\times P$ 的小块（Patch），每个 Patch 展平后通过线性层映射到 $d_{model}$ 维，于是图片就变成了长度为 $N=HW/P^2$ 的序列。随后加入位置编码，并送入 Transformer Encoder。

| 文本 Transformer | ViT |
| --- | --- |
| 一个 Token 通常是一个子词 | 一个 Token 是一个图像 Patch |
| Token Embedding 来自词表查找 | Patch Embedding 来自展平与线性投影 |
| 位置编码表示词元顺序 | 位置编码表示 Patch 的空间位置 |
| 常用特殊 Token 如 `[BOS]` | 常加入 `[CLS]` 汇总整张图的信息 |
| 注意力学习词元之间的关系 | 注意力学习远距离图像区域之间的关系 |

因此，ViT 并不是为图片重新发明一种注意力，而是改变了“如何把输入变成 Token”。经典 ViT 主要使用 Encoder：所有 Patch 可以双向互相关注，不需要 GPT 那样的因果掩码；分类时通常读取 `[CLS]` 的最终表示。它与 Decoder-Only 大语言模型的共同点是都对序列做注意力计算，主要区别则在输入 Token 的含义、训练目标以及是否需要因果生成。
...
这里的“只保留解码器”是一种便于理解的说法，但不能把原始 Transformer Decoder 原封不动地搬过来。原始 Decoder 每层包含“掩码自注意力、Cross-Attention、前馈网络”三个主要子层；GPT 没有独立的 Encoder 输出可供查询，因此会<strong>删除 Cross-Attention</strong>，保留因果自注意力与前馈网络等组件。更准确地说，GPT 使用的是<strong>由 Transformer Decoder 演化而来的、仅含因果自注意力的堆叠结构</strong>。
...
可以把它放到模型训练流程中理解：<strong>预训练</strong>让模型学会“语言中什么内容可能接在后面”，<strong>指令调优</strong>再用高质量的任务示例教模型“面对一条指令应该怎样回答”。它会更新模型参数，和只在输入中提供几个示例的 Few-shot Prompting 不同；它也不等于给模型实时补充新知识，主要目标是让模型学会任务格式、回答方式和指令遵循。

一条典型的指令调优数据可以包含三部分：`instruction`（要做什么）、`input`（可选的任务材料）和 `output`（理想回答）。大量不同任务的数据混合训练后，同一个模型才更容易迁移到未见过但表达方式相近的新指令。
...
[15] Dosovitskiy, A., Beyer, L., Kolesnikov, A., Weissenborn, D., Zhai, X., Unterthiner, T., ... & Houlsby, N. (2021). An image is worth 16x16 words: Transformers for image recognition at scale. In *International Conference on Learning Representations*.
