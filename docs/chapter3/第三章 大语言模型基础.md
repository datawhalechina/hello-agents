...
- 稳定化与归一化：将得到的所有分数除以缩放因子$\sqrt{d_{k}}$（$d_{k}$是$K$向量的维度）。当$d_{k}$增大时，未经缩放的点积分数通常会具有更大的绝对值，使Softmax进入饱和区并产生很小的梯度。除以$\sqrt{d_{k}}$可以控制分数的尺度、缓解Softmax饱和，从而让训练更加稳定；随后用Softmax函数将分数转换成总和为1的权重，完成归一化。
