#!/bin/bash
# ============================================================
# Restart script for GuessHistoricalFigure
...
echo ""
echo "╔══════════════════════════════════════════════╗"
echo "║   GuessHistoricalFigure — Restart Script     ║"
echo "╚══════════════════════════════════════════════╝"
echo ""
...
kill_pattern "GuessHistoricalFigure/frontend"
sleep 1

# ── Step 2: Start backend ──────────────────────────────────
info "Starting backend (port $BACKEND_PORT)..."
> "$BACKEND_LOG"   # clear log on each restart
cd "$BACKEND_DIR"
nohup "$VENV_PYTHON" main.py > "$BACKEND_LOG" 2>&1 &
BACKEND_PID=$!
