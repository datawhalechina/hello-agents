...
备份内容（v2）：
    ...
    pending_workout.json            仅在存在时
    external_model_settings.json    仅在存在时
    health.db                       仅在存在时，用 sqlite backup API 取一致快照
    health-imports/<文件名>          原始导入文件
    hr_streams/<文件名>              已保存训练的 1Hz 心率流

导出和恢复均按成员流式处理并校验 sha256。批量清空或恢复前会创建恢复点；
恢复时使用维护开关和文件锁，避免并发写入覆盖恢复结果。
"""
