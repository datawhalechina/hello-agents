...
| 🧠 **Memory System** | Daily first-start date tracking, daily cutover re-fetches dashboard snapshot; watchlist count changes trigger memory refresh **when prior watchlist records exist** (frontend still requests watchlist API in real-time) | ✅ |
...
- **Multi-Agent Collaboration**: Employs Reflection, ReAct, FunctionCall, PlanAndSolve and other paradigms; stock analysis page tabs can independently run streaming agents, AI chat assistant routes via coordinator on demand
...
- **Personalized Investment Analysis**: User preference storage (risk preference, investment style, sector preference), agents auto-inject preference parameters
...
Copy `.env.example` to `.env` and fill in your keys. Commonly used items are as follows (see `.env.example` comments for full details):
...
![Launch Frontend](images/启动前端服务.png)

![Launch Backend](images/启动后端服务.png)

![Screenshot 1](images/运行效果1.png)

![Screenshot 2](images/运行效果2.png)

![Screenshot 3](images/运行效果3.png)

![Screenshot 4](images/运行效果4.png)

![Screenshot 5](images/运行效果5.png)

![Screenshot 6](images/运行效果6.png)
...
├── README.md
├── README_EN.md
├── 功能修改方案.md                 # Feature modification requirements doc
├── 实现方案.md                     # Technical implementation doc
├── requirements.txt                # Root aggregated dependencies
├── main.ipynb                      # Jupyter experiment entry (optional)
├── data/                           # Data directory
│   ├── stock_cache/                #   File cache (independent JSON per stock)
│   └── memory/                     #   Memory system state persistence
├── outputs/                        # Sample reports/screenshots directory
├── src/                            # Course template notes
...
│   ├── general_advisor_agent.py    #   General investment advisor agent
│   ├── sentiment_agent.py          #   Sentiment analysis agent (streaming support)
│   ├── data_analysis_agent.py      #   Data analysis agent (streaming support)
│   ├── screener_agent.py           #   Stock screening agent
│   ├── trading_agent.py            #   Trade execution agent
│   ├── preference_injector.py      #   Preference injection module
│   └── tools/                      #   Custom tool wrappers
│
├── HelloAgents Optimized/          # 🧩 Multi-agent framework
│
├── skills/                         # 🔧 East Money MX skill wrappers
│
├── docker-compose.yml
├── DEPLOY.md
├── run_exe.py                      # exe packaging entry
└── .env.example
```
