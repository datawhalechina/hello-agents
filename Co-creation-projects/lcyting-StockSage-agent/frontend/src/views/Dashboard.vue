...
async function loadWatchlist() {
  ...
  try {
    ...
    if (res.code === 0 && res.data?.stocks) {
      ...
      // 并发获取每只自选股的实时行情
      const quotePromises = stocks.map(stock =>
        api
          .get(`/market/quote/${stock.code || stock.name}`)
          .then((r) => {
            if (!r || r.code !== 0 || r.data == null) return null
            return r.data
          })
          .catch(() => null)
      )
      const quotes = await Promise.all(quotePromises)
      watchlist.value = stocks.map((stock, i) => {
        const parsed = parseStockQuote(quotes[i])
        // 妙想自选股列表接口已带每只股票的价与涨跌幅；mx_data 单行解析失败时用其兜底（避免仅第一行表结构碰巧命中）
        const zPrice =
          stock.price != null && String(stock.price).trim() !== '' ? stock.price : null
        const zChange = parseWatchlistChangePct(stock.change_pct)
        const price = parsed?.price ?? zPrice ?? '--'
        const change =
        ...
        return {
          ...
          name: stock.name || parsed?.name || stock.code || stock.name || '--',
          price,
          change,
        }
      })
