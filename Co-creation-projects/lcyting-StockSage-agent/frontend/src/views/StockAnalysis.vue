  ...
  <div class="analysis-page">
    ...
    <template v-if="stockCode">
      ...
      <el-tabs v-model="activeTab" type="border-card" class="analysis-tabs">
        ...
        <!-- Tab 3: 舆情分析 -->
        <el-tab-pane label="舆情分析" name="sentiment">
          <div v-loading="sentLoading">
            <el-row :gutter="16" class="sentiment-summary" v-if="sentData">
              <el-col :span="8">
                <el-statistic title="新闻数量" :value="sentData.news_count || 0" />
              </el-col>
              <el-col :span="8">
                <el-statistic title="研报数量" :value="sentData.report_count || 0" />
              </el-col>
              <el-col :span="8">
                <el-statistic title="公告数量" :value="sentData.announce_count || 0" />
              </el-col>
            </el-row>

            <!-- 新闻列表 -->
            <el-card shadow="hover" v-if="newsItems.length > 0" class="sent-card">
              <template #header>
              ...
              <div class="news-scroll">
                <div
                  ...
                  :key="i"
                  class="sent-news-item"
                  role="button"
                  tabindex="0"
                  @click="openNewsDetail(item)"
                  @keyup.enter="openNewsDetail(item)"
                >
                  <div class="sent-news-title">{{ item.title }}</div>
            ...
            <el-empty v-else-if="!sentLoading" description="暂无舆情数据" :image-size="60" />
          </div>
        </el-tab-pane>

        <!-- Tab 4: 巴菲特评估（流式生成全文，不展示框架/填空模板） -->
        <el-tab-pane label="巴菲特评估" name="buffett">
          <div class="buffett-section">
            <div v-if="stockCode" class="buffett-top-tip-row">
              <el-text type="info" size="small">
                请使用页头「生成巴菲特评估报告」流式生成全文（Markdown）；需服务端配置 LLM_API_KEY。
              </el-text>
            </div>

            <div
              v-if="stockCode && !buffettStreaming && !buffettAiMarkdown.trim()"
              class="buffett-generate"
            >
              <el-empty
                description="点击页头「生成巴菲特评估报告」，将实时流式生成报告"
                :image-size="72"
              />
            </div>

            <div v-if="buffettStreaming && buffettStreamRaw" class="buffett-stream-wrap">
              <div class="buffett-stream-label">正在生成（实时预览 · Markdown 原文）</div>
            ...
            <div v-if="buffettAiMarkdown.trim() && !buffettStreaming" class="buffett-ai-wrap">
              <div class="buffett-ai-header">
                ...
                <div class="buffett-ai-header-actions">
                  <el-button size="small" @click="runBuffettAiReportStream" :loading="buffettStreaming">
                    重新生成
                  </el-button>
                  <el-button size="small" type="primary" plain @click="downloadBuffettReport">
            ...
            <el-alert
              v-if="buffettAiError && !buffettStreaming"
              type="warning"
              :closable="false"
              show-icon
              :title="buffettAiError"
              class="buffett-error-alert"
            />
          </div>
        </el-tab-pane>
        ...
        <el-tab-pane label="AI分析报告" name="report">
          <div class="report-section">
            <div v-if="!reportData && !reportLoading" class="report-generate">
              <el-empty description="点击生成AI分析报告" :image-size="80">
            ...
            <div v-loading="reportLoading" v-if="reportLoading" class="report-loading">
              <el-icon class="is-loading" :size="32"><Loading /></el-icon>
              <p>正在生成分析报告，请稍候...</p>
              <el-text type="info">AI正在综合分析行情、财务、舆情数据</el-text>
            </div>

            <div v-if="reportData" class="report-content">
              <div class="report-header">
                <h3>{{ stockName }}（{{ stockCode }}）分析报告</h3>
...
/** 将简易 Markdown 转为 HTML（与 AI 报告展示一致） */
function formatMarkdownLikeToHtml(text) {
  if (!text) return ''
  return String(text)
    .replace(/\n/g, '<br>')
...
function parseProfileData(data) {
  ...
  const longLabel = (label) =>
    /简介|范围|业务|介绍|概况|描述|主营/.test(String(label))
...
/** 第二阶段：财务 / 概况 / 股东 / 舆情（与行情并行解耦，_tab 内单独 loading） */
async function loadSecondaryAnalysis(seq) {
  ...
  try {
    const [finRes, profileRes, holdersRes, sentRes] = await Promise.all([
      ...
      api.get(`/news/sentiment/${stockCode.value}`).then(unwrapData).catch(() => null),
    ])
    ...
    if (sentRes?.success) {
      sentData.value = {
        news_count: sentRes.news_items?.length || 0,
        report_count: sentRes.report_items?.length || 0,
        announce_count: sentRes.announce_items?.length || 0,
      }
      newsItems.value = [
        ...(sentRes.news_items || []),
        ...(sentRes.report_items || []),
        ...(sentRes.announce_items || []),
      ].slice(0, 20)
    } else {
      sentData.value = null
      newsItems.value = []
...
/** 头部唯一入口：切换到巴菲特 Tab 并流式生成报告 */
async function handleBuffettQuick() {
  activeTab.value = 'buffett'
  await runBuffettAiReportStream()
}
  ...
  try {
    ...
    while (true) {
      ...
      while ((nl = buf.indexOf('\n')) >= 0) {
        ...
        if (obj.type === 'delta' && obj.text) {
          buffettStreamRaw.value += obj.text
    ...
    if (tail) {
      try {
        ...
        if (obj.type === 'error') throw new Error(obj.message || '生成失败')
        if (obj.type === 'delta' && obj.text) buffettStreamRaw.value += obj.text
      } catch (e) {
...
async function generateReport() {
  ...
  try {
    const res = await api.post(`/analysis/report/${stockCode.value}`)
    if (res.code === 0 && res.data?.report) {
      reportData.value = res.data.report
      // 将报告内容格式化为HTML
      const content = res.data.report.content || res.data.report.summary || ''
      reportContent.value = formatMarkdownLikeToHtml(content)
    } else {
