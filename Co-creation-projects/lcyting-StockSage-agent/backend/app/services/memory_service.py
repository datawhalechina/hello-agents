"""
记忆系统 — 基于 HelloAgents 框架的每日数据缓存与过期管理

记录每天第一次打开后端的时间日期，若超过第二天则重新获取数据并删除前一天所有数据。
每天首次启动时三线程并行获取指数、自选、热点资讯数据，保存到记忆系统等待第二天刷新。
若自选股数量变化则重新刷新。

记忆系统采用 hello_agents 框架的 MemoryManager 作为底层存储。
"""
