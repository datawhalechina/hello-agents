...
def iter_buffett_ai_report_events(stock_code: str, stock_name: str = "") -> Iterator[Dict[str, Any]]:
    """供 NDJSON 流式响应：依次产出 meta / delta* / done 或 error。"""
    ...
    code = (stock_code or "").strip()
    yield {"type": "meta", "stock_code": code, "stock_name": prep.get("name") or code}

    llm = make_buffett_llm_client()
    try:
        for delta in llm.think(prep["messages"], temperature=0.35):
            if delta:
