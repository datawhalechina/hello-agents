...
from ...models.schemas import (
    SaveHistoryRequest, HistoryListResponse, HistoryItemResponse,
    HistoryDetailResponse, DeleteResponse
)
from ...database import (
    save_trip_history, list_trip_history, get_trip_history, delete_trip_history
)
...
async def save_history(req: SaveHistoryRequest, authorization: str = Header(None)):
    """保存生成的行程到历史记录"""
    ...
    return HistoryItemResponse(success=True, message="保存成功", history_id=hid)
...
async def list_history(
    ...
    return HistoryListResponse(success=True, records=records)
...
async def get_history(history_id: int, authorization: str = Header(None)):
    """获取单条历史记录的完整行程数据"""
    user = require_auth(authorization)
    ...
    return HistoryDetailResponse(success=True, record={
        "id": record["id"],
        "city": record["city"],
...
async def delete_history(history_id: int, authorization: str = Header(None)):
    """删除一条历史记录"""
    user = require_auth(authorization)
